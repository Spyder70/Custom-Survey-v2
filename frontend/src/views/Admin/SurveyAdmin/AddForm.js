...
const DraggableBlock = ({ id, formIndex, name, type, index, moveBlock, isRequired, numButtons, buttonNames, options }) => {
  ...
  const [, drop] = useDrop({
    ...
    hover(item, monitor) {
      ...
      moveBlock(item.index, index, item.formIndex, formIndex);
      item.index = index;
  ...
  return (
    <div
      ...
      className={`draggable-block ${isDragging ? 'dragging' : ''} ${isButton ? 'button-block' : ''} ${isFormname ? 'formname-block' : ''} ${isCheckbox ? 'checkbox-block' : ''} ${isRadio ? 'radio-block' : ''} ${isDropdown ? 'dropdown-block' : ''}`}
    >
      <div>
        {isFormname ? (
    ...
    <DndProvider backend={HTML5Backend}>
      <div style={{ textAlign: 'center', maxWidth: '600px', margin: 'auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '5px' }}>
          <button onClick={addBlock} style={{ padding: '12px', backgroundColor: '#4CAF50', color: 'white', border: 'none', marginRight: '10px' }}>Add Block</button>
          ...
          <div key={formIndex}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '10px' }}>
              ...
              <button
                ...
                style={{
                  backgroundColor: !form.isActive ? 'red' : '',
                  color: 'white',
                  padding: '8px',
                  borderRadius: '4px',
                  marginLeft: '10px',
